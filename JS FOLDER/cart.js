...
function parsePrice(priceText) {
    // Remove currency symbols, spaces, then commas, keep digits and dot only
...
// FIX: create popup dynamically if it doesn't exist on the page
let addPopUp = el('.addToCartPopUp');
if (!addPopUp) {
    addPopUp = document.createElement('div');
    addPopUp.className = 'addToCartPopUp';
    addPopUp.id = 'addToCartPopUp';
...
AllAddButtons.forEach(add => {
    add.addEventListener('click', () => {
        ...
        if (addPopUp) {
            ...
            addPopUp.style.backgroundColor = "green";
            addPopUp.textContent = "Added to Cart!";
            setTimeout(() => { addPopUp.style.display = "none"; }, 1000);
        }
    });
});
...
AllTrashButtons.forEach(trash => {
    trash.addEventListener('click', () => {
        ...
        if (addPopUp) {
            addPopUp.style.display = "flex";
            addPopUp.textContent = "Removed from Cart!";
            addPopUp.style.backgroundColor = "red";
            setTimeout(() => { addPopUp.style.display = "none"; }, 1000);
        }
...
function syncPageButtons() {
    AllAddButtons.forEach(add => {
        ...
        if (cartArray.some(item => item.name === productName)) {
            eachAddButton.style.backgroundColor = "orange";
...
function showConfirmDetailsPopup(name, contact, address, payment, total, checkedNames) {
    ...
    // No — go back to checkout form
    document.getElementById('confirmDetailsNo').addEventListener('click', () => {
        popup.remove();
    });

    // Yes — remove cart items, show loading then success
...
function showOrderLoading(name, payment, total) {
    ...
    // animate dots then show success after 1.5s
